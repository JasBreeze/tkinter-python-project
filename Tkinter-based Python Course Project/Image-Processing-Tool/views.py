...
class ModernEditorView(tk.Tk):
    ...
    def _build_text_panel(self):
        ...
        # 文字输入
        text_entry = self._create_labeled_entry("文字:", command=lambda e: self.controller._update_text_preview())
        # 保存引用，供控制器访问
    ...
    def _build_batch_watermark_panel(self):
        ...
        # 字体大小和透明度
        size_opacity_container = ttk.Frame(content_frame)
        size_opacity_container.pack(fill=tk.X, pady=1, padx=3)
        
        # 字体大小
        size_frame = ttk.Frame(size_opacity_container)
        size_frame.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=2)
        ...
        # 透明度
        opacity_frame = ttk.Frame(size_opacity_container)
        opacity_frame.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=2)
        ...
        offset_x_frame.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=2)
        ...
        offset_y_frame.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=2)
