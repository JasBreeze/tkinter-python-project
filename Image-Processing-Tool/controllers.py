...
class EditorController:
    ...
    def open_image(self):
        ...
        if path:
            try:
                self.filepath = path
                # 尝试打开图片并处理带有透明通道的调色板图像
                img = Image.open(path)
                # 先检查是否是带有透明通道的调色板图像，如果是则先转换为RGBA
    ...
    def _init_batch_watermark_vars(self):
        ...
        self.batch_watermark_vars = {
            ...
            # 图片水印设置
            'image_watermark_path': tk.StringVar(value=""),
            'image_watermark_scale': tk.IntVar(value=50),
            'image_watermark_opacity': tk.IntVar(value=180),
    ...
    def _batch_watermark_select_image(self):
        """选择图片水印文件"""
        ...
        if file_path:
            vars['image_watermark_path'].set(file_path)
    ...
    def _batch_add_watermark_in_thread(self):
        ...
        for i, file_path in enumerate(selected_files):
            try:
                ...
                # 获取水印类型
                watermark_type = vars['watermark_type_var'].get()
                ...
                if watermark_type == "text":
                    ...
                    # 图片水印处理
                    img_watermark_path = vars['image_watermark_path'].get()
                    if not img_watermark_path:
                        raise ValueError("请选择图片水印")
                    
                    # 打开水印图片
                    watermark_img = Image.open(img_watermark_path).convert("RGBA")
                    
                    # 调整水印图片大小
                    scale = vars['image_watermark_scale'].get() / 100
                    ...
                    # 调整水印透明度
                    opacity = vars['image_watermark_opacity'].get() / 255
                    if opacity < 1.0:
                        # 创建新的水印图层，调整透明度
                    ...
                    layer.paste(watermark_img, (x, y), watermark_img)
                    img_with_watermark = Image.alpha_composite(img, layer).convert("RGB")
